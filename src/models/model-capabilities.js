...
function findLocalCapabilities(provider, model) {
  ...
  const exact = LOCAL_CAPABILITIES.find(
    (item) =>
      normalize(item.provider) === normalizedProvider && normalize(item.model) === normalizedModel,
  );
  if (exact) {
    return { ...exact };
  }

  const loose = LOCAL_CAPABILITIES.find(
    (item) =>
...
function findRemoteModel(models, model) {
  const normalizedModel = normalize(model);
  return (
    models.find((item) => normalize(item.id) === normalizedModel) ||
...
function findLiteLLMEntry(data, provider, model) {
  ...
  return (
    entries.find(([name]) => normalize(name) === normalizedModel) ||
    entries.find(
      ([name, value]) =>
