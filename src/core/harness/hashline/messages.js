...
/**
 * Numbered `LINE:TEXT` rows around `anchorLines` (±`MISMATCH_CONTEXT`),
 * `*`-marking anchors, `...` between non-adjacent runs. Out-of-range anchors
 * contribute no rows.
 *
 * @param {readonly number[]} anchorLines
 * @param {readonly string[]} fileLines
 * @returns {string[]}
 */
...
export const BARE_BODY_OVERLAP_WARNING = `Dropped a bare hunk overlapped by the concrete hunk after it. One \`SWAP N${HL_RANGE_SEP}M:\` hunk per range — the body is the final content, never old+new.`;
...
/** `delete_block N` resolved to a single line — usually a mis-anchor. */
export function blockSingleLineMessage(line, op = 'replace') {
  const phrase = op === 'delete' ? `DEL.BLK ${line}` : `SWAP.BLK ${line}:`;
  const fallback = op === 'delete' ? `DEL ${line}` : `SWAP ${line}`;
  return (
    `\`${phrase}\` resolved to a single line (line ${line}). ` +
    `Tree-sitter block ops target multi-line constructs; for a single line, use \`${fallback}\` directly.`
  );
}
 ...
 * parse error). Appends a `formatAnchoredContext` preview when
 * `fileLines` is given. `insert_after_block N:` never reaches this — it is
 * lowered to plain `insert after N:` instead.
 */
 ...
 * `RECOVERY_SESSION_CHAIN_WARNING` — the 3-way merge refused, the
...
/**
 * `Recovery`: every anchor in a pure-delete edit set was already gone from
 * the live file (deleted by an external write). The delete is a no-op — the
 * targeted lines no longer exist — so recovery succeeds with a warning
 * rather than hard-failing.
 */
export const RECOVERY_GONE_DELETE_WARNING =
  'Recovered by skipping delete edit(s) whose target lines were already removed from the live file (file changed externally since the tagged read). The deletion is already satisfied.';
 ...
 */
export function pathRecoveredFromTagMessage(authoredPath, resolvedPath, tag) {
  return (
...
/**
 * Content preview handed to `unseenLinesMessage`. `lines` are the
 ...
 */
export class UnseenLinesReveal {
  constructor(lines = [], truncated = false) {
    this.lines = lines;
...
export function unseenLinesMessage(
  ...
  return (
    ...
    `${HL_FILE_PREFIX}path${HL_FILE_HASH_SEP}tag${HL_FILE_SUFFIX} header — a straight retry now succeeds because those lines are marked as seen.`
  );
}
