...
export class TextToolParser {
  ...
  #hasNamedXMLCandidate(text) {
    ...
    for (const tool of tools) {
      if (tool?.name && text.includes(`<${tool.name}>`)) { return true; }
    }
    for (const [alias, runtimeName] of Object.entries(NAMED_XML_TOOL_ALIASES)) {
      if (this.#toolRegistry?.has?.(runtimeName) && text.includes(`<${alias}>`)) { return true; }
    }
    return false;
  }
  ...
  #scanForToolIntentWithoutParse(text) {
    ...
    while ((fence = fenceRegex.exec(text)) !== null) {
      ...
      if (body.startsWith('{') && body.endsWith('}')) {
        ...
        if (!parsed || typeof parsed !== 'object') {
          return { tag: 'tool_code_block_invalid_json', opening: '```tool', closing: '```', hint: 'The JSON inside the tool code fence could not be parsed.', sample: body.substring(0, 160) };
        }
      }
    }
    ...
    if (callMatch) {
      ...
      if (!found) {
        return { tag: 'call_unbalanced_json', opening: 'CALL ' + callMatch[1] + '({', closing: '(no matching })', hint: 'CALL ' + callMatch[1] + ' must contain a balanced JSON object.', sample: text.substring(callMatch.index, Math.min(callMatch.index + 160, text.length)) };
      }
      const jsonText = text.substring(braceIdx, found.endIdx);
      const parsed = this.#safeJSONParse(jsonText);
      if (!parsed || typeof parsed !== 'object') {
        return { tag: 'call_invalid_json', opening: 'CALL ' + callMatch[1] + '({', closing: '})', hint: 'CALL ' + callMatch[1] + ' contains text that cannot be parsed as JSON.', sample: jsonText.substring(0, 160) };
      }
    }
  ...
  #extractRecoveredValue(raw) {
    ...
    if (value.startsWith('"') || value.startsWith("'")) {
      ...
      if (value.length >= 2 && value[value.length - 1] === openQuote) { return value.slice(1, -1); }
      for (let i = value.length - 1; i > 0; i--) {
        if (value[i] === openQuote && value[i - 1] !== '\\') { return value.slice(1, i); }
      }
      ...
      try {
        ...
        while (i < str.length) {
          ...
          if (ch === '"') {
            ...
            while (i < str.length) {
              const c = str[i];
              if (escaped) { normalized += c; escaped = false; i++; continue; }
              if (c === '\\') { normalized += c; escaped = true; i++; continue; }
              if (c === '"') { normalized += c; i++; break; }
              if (c === '\n') { normalized += '\\n'; i++; continue; }
        ...
        fixed = fixed.replace(/([{,]\s*)([A-Za-z_][\w-]*)\s*:/g, '$1"$2":').replace(/,\s*([}\]])/g, '$1');
  ...
  #toolCallsFromJSON(json, source, startIndex = 0) {
    ...
    const directName = json.name || json.tool || json.tool_name || (typeof json.action === 'string' ? json.action : '');
    if (directName) {
      const originalArgs = json.arguments || json.args || json.params || Object.fromEntries(
        Object.entries(json).filter(([key]) => !['name', 'tool', 'tool_name', 'action', 'source', 'id'].includes(key))
      );
    ...
    const action = json.action && typeof json.action === 'object' && !Array.isArray(json.action) ? json.action : json;
    const entries = Object.entries(action);
    if (entries.length !== 1) { return []; }
    const [rawName, originalArgs] = entries[0];
    const { name, args } = this.#normalizeJSONToolCall(rawName, originalArgs);
    if (!this.#toolRegistry?.has?.(name)) { return []; }
    return [{ id: 'call_' + Date.now() + '_' + startIndex, name, arguments: args && typeof args === 'object' && !Array.isArray(args) ? args : {}, source }];
  }

  #resolveToolName(name) {
    const rawName = String(name || '').replace(/^\//, '');
    if (this.#toolRegistry?.has?.(rawName)) { return rawName; }
    const underscored = rawName.replace(/-/g, '_');
    if (this.#toolRegistry?.has?.(underscored)) { return underscored; }
    const snakeCase = rawName.replace(/([a-z0-9])([A-Z])/g, '$1_$2').replace(/-/g, '_').toLowerCase();
    if (this.#toolRegistry?.has?.(snakeCase)) { return snakeCase; }
    return rawName;
  }
  ...
  #normalizeJSONToolCall(rawName, originalArgs) {
    ...
    const aliases = {
      ...
      fetch_url: 'web_fetch', browser_fetch: 'web_fetch',
      plan: 'brainstorm', plan_solution: 'brainstorm',
    };
    ...
    if (resolvedName === 'shell' && !args.command && (args.cmd || args.path || args.commands || args.code || args.arg0 || args.value)) {
      return { name: resolvedName, args: { ...args, command: args.cmd || args.path || args.commands || args.code || args.arg0 || args.value } };
    }
    if (resolvedName === 'brainstorm') {
      return { name: resolvedName, args: { problem: args.problem || args.steps || args.plan || args.value || 'Plan the requested implementation before editing files.' } };
    }
    if (resolvedName === 'web_search') {
      return { name: resolvedName, args: { ...args, query: args.query || args.q || args.text || args.value || args.question } };
    }
    if (resolvedName === 'web_fetch') {
      return { name: resolvedName, args: { ...args, url: args.url || args.href || args.link || args.value } };
    }
    return { name: resolvedName, args: normalizeToolArgumentAliases(resolvedName, args) };
  }

  #normalizeLooseArgs(toolName, args) {
    if (toolName === 'list_dir' && args.value && !args.path) { return { ...args, path: args.value }; }
    if (toolName === 'list_dir' && !args.path) { return { ...args, path: '.' }; }
    if (toolName === 'read_file' && args.value && !args.path) { return { ...args, path: args.value }; }
    if (toolName === 'shell' && args.value && !args.command) { return { ...args, command: args.value }; }
    if (toolName === 'web_search' && args.value && !args.query) { return { ...args, query: args.value }; }
    if (toolName === 'web_fetch' && args.value && !args.url) { return { ...args, url: args.value }; }
    return args;
  }

  #castParam(value, type) {
    if (type === 'boolean') { return ['true', 'yes', '1', 'on'].includes(value.toLowerCase()); }
    if (type === 'number' || type === 'integer') { const num = Number(value); return isNaN(num) ? value : num; }
    if (type === 'array') { try { return JSON.parse(value); } catch { return value.split(/[,;]/).map(s => s.trim()); } }
    return value;
  }
  ...
  #parseToolCodeArgs(argsText) {
    ...
    while ((match = stringRegex.exec(text)) !== null) {
      ...
      if (key) { args[key] = value; } else { args.positional.push(value); }
    }
    return args;
  }
  ...
  #normalizeToolCodeArgs(toolName, parsedArgs) {
    if (toolName === 'list_dir') { return { path: parsedArgs.path ?? parsedArgs.positional[0] ?? '.' }; }
    if (toolName === 'read_file') { const path = parsedArgs.path ?? parsedArgs.file ?? parsedArgs.file_path ?? parsedArgs.positional[0]; return path ? { path } : null; }
    if (toolName === 'write_file') { const path = parsedArgs.path ?? parsedArgs.file ?? parsedArgs.file_path ?? parsedArgs.positional[0]; const content = parsedArgs.content ?? parsedArgs.text ?? parsedArgs.positional[1]; return path && content !== undefined ? { path, content } : null; }
    if (toolName === 'shell') { const command = parsedArgs.command ?? parsedArgs.cmd ?? parsedArgs.positional[0]; return command ? { command } : null; }
    if (toolName === 'web_search') { const query = parsedArgs.query ?? parsedArgs.q ?? parsedArgs.text ?? parsedArgs.positional[0]; return query ? { query } : null; }
    if (toolName === 'web_fetch') { const url = parsedArgs.url ?? parsedArgs.href ?? parsedArgs.link ?? parsedArgs.positional[0]; return url ? { url } : null; }
    if (toolName === 'brainstorm') { const problem = parsedArgs.problem ?? parsedArgs.positional[0] ?? 'Plan the requested implementation before editing files.'; return { problem }; }
    return {};
  }
