...
function createWriteMemoryTool() {
  return {
    name: 'write_memory',
    description:
      'Write a structured memory entry to the agent memory system. Use this to store important information for future tasks. Choose the appropriate type: user (user preferences), feedback (user feedback), project (project-specific knowledge), reference (reference material).',
...
function createReadMemoryTool() {
  return {
    name: 'read_memory',
    description:
      'Read the full content of a memory entry by ID. Use this when you need detailed information from a specific memory. You can get memory IDs from list_memory or retrieve_memory tools.',
    category: ToolCategory.FILESYSTEM,
    params: {
...
function createRetrieveMemoryTool() {
  return {
    ...
    description:
      'Retrieve relevant memories based on a query. The system will search through all memories and return the most relevant ones. Use this to find information that may be useful for the current task.',
    category: ToolCategory.FILESYSTEM,
    params: {
      ...
      forceVerification: {
        type: 'boolean',
        description: 'Force verification of results',
        default: false,
      },
    },
    required: ['query'],
...
function createListMemoryTool() {
  return {
    name: 'list_memory',
    description:
      'List all memory entries with basic information. Use this to see what memories are available.',
    category: ToolCategory.FILESYSTEM,
    params: {
      type: {
        type: 'string',
...
function createVerifyMemoryTool() {
  return {
    name: 'verify_memory',
    description:
      'Verify if a memory entry is still valid. Checks if file/function/flag references still exist and match expected values.',
    category: ToolCategory.FILESYSTEM,
    params: {
