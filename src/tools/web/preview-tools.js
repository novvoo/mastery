...
function createPreviewStartTool() {
  return {
    name: 'preview_start',
    description: 'Start a local preview for generated HTML or Node projects. Static HTML is served from the workspace over localhost; Node projects start a package script/dev server and return the preview URL.',
    category: ToolCategory.WEB,
    params: {
      target: { type: 'string', description: 'HTML file or project directory relative to the working directory. Defaults to current workspace.' },
      kind: { type: 'string', enum: ['auto', 'static', 'node'], description: 'Preview type. auto detects from target/package.json.' },
      command: { type: 'string', description: 'Optional Node command, e.g. npm run dev. Used for kind=node.' },
