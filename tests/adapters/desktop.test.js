...
describe('MainProcessIPCAdapter', () => {
  ...
  test('应该在 Desktop IPC 中本地处理 /debug 命令', async () => {
    ...
    const result = await mockIpcMain.simulateHandle('agent:processInput', mockEvent, {
      input: '/debug on',
      options: {},
    });
    ...
    expect(sentMessages.some((message) => message.channel === RuntimeEvent.STATUS_UPDATE)).toBe(
      true,
    );
  });
  ...
  test('应该为 Desktop preload 直接注册 preview invoke 通道', async () => {
    ...
    adapter.attachEngine(mockEngine);
    adapter.registerHandler('preview:start', async (options = {}) =>
      startPreview({
        workingDirectory: root,
        ...options,
      }),
    );
    adapter.registerHandler('preview:list', async () => ({
      success: true,
    ...
    try {
      ...
      const listResult = await mockIpcMain.simulateHandle('preview:list', { sender: { id: 123 } });
      expect(listResult.success).toBe(true);
      expect(
        listResult.previews.some((preview) => preview.session_id === startResult.session_id),
      ).toBe(true);
...
describe('RendererProcessIPCAdapter', () => {
  ...
  test('应该正确断开连接', async () => {
    ...
    expect(adapter.isConnected).toBe(false);
    expect(mockIpcRenderer.sentMessages.some((m) => m.channel === IPCMessageType.DISCONNECT)).toBe(
      true,
    );
  });
});
...
describe('Desktop Integration', () => {
  ...
  test('IPC 消息应该正确序列化和反序列化', () => {
    const originalMessage = new IPCMessage(
