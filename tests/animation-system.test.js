...
describe('desktop animation system', () => {
  test('selects semantic motion modes from runtime state', () => {
    ...
    expect(getAnimationMode({ status: 'running', messages: [{ type: 'thinking' }] })).toBe(
      MOTION_MODES.THINKING,
    );
    expect(
      getAnimationMode({ status: 'running', messages: [{ type: 'tool', toolName: 'read_file' }] }),
    ).toBe(MOTION_MODES.TOOL_USE);
    expect(getAnimationMode({ status: 'running', messages: [{ type: 'assistant_stream' }] })).toBe(
      MOTION_MODES.RESPONDING,
    );
    expect(getAnimationMode({ status: 'needs_user_input', messages: [] })).toBe(
      MOTION_MODES.WAITING,
    );
  ...
  test('class helpers produce stable CSS hooks', () => {
    ...
    expect(getMotionClassNames(MOTION_MODES.THINKING, { reducedMotion: true })).toContain(
      'agent-motion--reduced',
    );
