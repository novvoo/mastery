...
import {
  ...
  Patch, Section, OP_SWAP, OP_DEL, OP_INS_PRE, OP_INS_POST,
  computeTag,
...
describe('Multi-language Rename: Python', () => {
  ...
  test('rename function across multiple files with import chains', async () => {
    ...
    const edits = [
      // helpers.py: 定义处
      { file: 'src/utils/helpers.py', patch: `[src/utils/helpers.py#${computeTag(files['src/utils/helpers.py'])}]\nSWAP 1.=4:\n+def compute_item_total(items):\n+    """Calculate total from items."""\n+    return sum(item.price for item in items)\n+\n` },
      // order.py: import 处
      { file: 'src/services/order.py', patch: `[src/services/order.py#${computeTag(files['src/services/order.py'])}]\nSWAP 1.=1:\n+from utils.helpers import compute_item_total, format_result\n` },
      { file: 'src/services/order.py', patch: `[src/services/order.py#${computeTag(files['src/services/order.py'])}]\nSWAP 5.=5:\n+        total = compute_item_total(items)\n` },
      // handler.py: 别名 import 处
      { file: 'src/api/handler.py', patch: `[src/api/handler.py#${computeTag(files['src/api/handler.py'])}]\nSWAP 1.=1:\n+from utils.helpers import compute_item_total as compute_total\n` },
    ];
...
describe('Multi-language Rename: Rust', () => {
  ...
  test('rename trait and its implementation across files', async () => {
    const files = {
      ...
      'src/traits/mod.rs': [
        'mod old_processor;',
        'pub use old_processor::OldProcessor;',
      ].join('\n') + '\n',
  ...
  test('rename type alias through re-export chain', async () => {
    const files = {
      ...
      'src/lib.rs': [
        'mod types;',
        'pub use types::{OldUserMap, OldResult};',
        '',
        'pub mod services;',
      ].join('\n') + '\n',
      'src/services/mod.rs': [
