...
describe('Runtime 完整流程端到端测试', () => {
  ...
  describe('完整生命周期流程', () => {
    it('应该完成完整的初始化 -> 工具注册 -> 执行 -> 清理流程', async () => {
      ...
      // 验证初始化事件
      const initEvents = capturedEvents.filter(
        (e) =>
      ...
      // 验证工具注册事件
      const toolRegisterEvents = capturedEvents.filter((e) => e.type === RuntimeEvent.TOOL_LOADED);
      ...
      // 验证清理事件
      const disposeEvents = capturedEvents.filter((e) => e.type === RuntimeEvent.AGENT_STOP);
    ...
    it('应该正确处理配置更新流程', async () => {
      ...
      // 验证配置变更事件
      const configEvents = capturedEvents.filter((e) => e.type === RuntimeEvent.CONFIG_CHANGE);
    ...
    it('应该正确处理内存更新流程', async () => {
      ...
      // 验证内存更新事件
      const memoryEvents = capturedEvents.filter((e) => e.type === RuntimeEvent.MEMORY_UPDATE);
  ...
  describe('错误处理流程', () => {
    it('应该正确处理工具执行错误', async () => {
      ...
      // 验证错误事件
      const errorEvents = capturedEvents.filter((e) => e.type === RuntimeEvent.TOOL_ERROR);
    ...
    it('应该正确处理模型提供者缺失错误', async () => {
      ...
      try {
        ...
        const hasKeyword =
          error.message.includes('Model provider') || error.message.includes('模型提供者');
  ...
  describe('状态管理流程', () => {
    ...
    it('应该正确追踪状态变更事件', async () => {
      ...
      // 检查状态更新事件
      const statusEvents = capturedEvents.filter((e) => e.type === RuntimeEvent.STATUS_UPDATE);
...
describe('Desktop IPC 适配器完整流程端到端测试', () => {
  ...
  describe('主进程 IPC 适配器完整流程', () => {
    ...
    it('应该正确处理引擎附加和事件转发', async () => {
      ...
      // 验证事件被捕获
      const agentEvents = capturedEvents.filter((e) => e.type === RuntimeEvent.AGENT_START);
  ...
  describe('渲染进程 IPC 适配器完整流程', () => {
    it('应该完成完整的渲染进程 IPC 生命周期', async () => {
      ...
      // 模拟接收事件
      const eventMessage = new IPCMessage(
...
describe('集成场景端到端测试', () => {
  ...
  describe('Runtime + Plugin + Event 集成', () => {
    it('应该完成 Runtime、插件和事件的完整集成流程', async () => {
      ...
      // 验证事件流
      const toolEvents = capturedEvents.filter(
        (e) =>
